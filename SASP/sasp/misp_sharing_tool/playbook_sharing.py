...
attr_map = {
    ...
    "id": "id",        # ZAPNUT KORELACI - spravit docker komponentu, correlate ID nic ine
    "impact": "impact",
    "modified": "modified",
...
attr_map_sappan = {
    ...
    "id": "HasId",        # ZAPNUT KORELACI - spravit docker komponentu, correlate ID nic ine
}
...
def send_playbook_cacao(misp_inst):
    ...
    obj.add_attribute('playbook', value=path.name, data=path, disable_correlation=True)
    ...
    if('playbook_types' in data):
        ...
        tmp = missing_val_handler('playbook_types', True)
        if (tmp == None):
            return
    ...
    for l in data['labels']:
        obj.add_attribute('label', value=l, disable_correlation=True)
...
def send_playbook_sappan(misp_inst):
    ...
    if('HasVersion' in data['printouts']):
        ...
        tmp = missing_val_handler('HasVersion', True)
        if (tmp == None):
            return
        obj.add_attribute('playbook-standard', value=tmp)
    ...
    if('HasPlaybookFocus' in data["printouts"]):
        ...
        tmp = missing_val_handler('hasPlaybookFocus', True)
        if (tmp == None):
            return
        obj.add_attribute('playbook-standard', value=tmp, disable_correlation=True)
